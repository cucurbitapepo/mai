...
int convert_str_to_long_long(const char * str, long long * result, int base)
{
    char * endptr;
...
int calculation(const char *postfix, long long *result)
{
...
int creating_error_file(const char * filename, char ** file_error)
{
    char * prefix = "err_";
...
int read_string(FILE * input, char ** infix, int * capacity_infix)
{
    int idx = 0;
    char c = fgetc(input);
